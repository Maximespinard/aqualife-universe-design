document.addEventListener('DOMContentLoaded', function () {
  ...
  const mobileFilterSelect = document.getElementById('mobile-filter-selector');

  if (mobileFilterSelect) {
    // Set initial value to match the active tab
    const activeTab = document.querySelector('.filter-tab.active');
    if (activeTab) {
      mobileFilterSelect.value = activeTab.getAttribute('data-filter');
    }
    ...
    mobileFilterSelect.addEventListener('change', function () {
      ...
      const selectedPanel = document.getElementById(selectedFilter + '-panel');
      if (selectedPanel) {
        selectedPanel.classList.add('active');
      }

      // Update the desktop tabs to match (for consistency when switching viewports)
  ...
  filterTabs.forEach((tab) => {
    tab.addEventListener('click', () => {
      ...
      // Also update mobile dropdown to match
      if (mobileFilterSelect) {
        mobileFilterSelect.value = filterType;
      }
    });
  });
